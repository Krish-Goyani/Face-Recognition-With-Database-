...
class App():
    def __init__(self):
        ...
        self.main_window.title("Smart Attendance System - Computer Vision")
    ...
    def login(self):
        unkown_img_path = "temp.jpg"
        cv2.imwrite(unkown_img_path, self.most_recent_capture_arr)

        name = str(subprocess.check_output(['face_recognition',self.db_dir,unkown_img_path]))
        name = name.split(",")[1][:-5]
        print(name)
    ...
    def logout(self):
        ...
        if name in ['unknown_person', 'no_persons_found']:
            ...
            util.msg_box('Have a nice day!', 'Goodbye, {}.'.format(name))
            with open(self.log_path, 'a') as f:
                f.write('{},{},out\n'.format(name, datetime.datetime.now()))
                f.close()
